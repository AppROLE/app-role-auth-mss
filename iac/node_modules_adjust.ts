...
export function nodeModulesAdjust(): void {
  ...
  execSync('npm init -y');
  execSync('npm install mongoose uuid dotenv nodemailer @aws-sdk/client-cognito-identity-provider busboy aws-sdk');
  execSync('npm install busboy');

  // ensure that busboy is installed in the correct directory
  const busboyPath = path.join(destinationDirectory, 'node_modules', 'busboy');
  if (!fs.existsSync(busboyPath)) {
    execSync('npm install busboy', { cwd: destinationDirectory });
  }
