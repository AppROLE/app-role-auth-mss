...
export class ForgotPasswordUseCase {
  ...
  async execute(email: string) {
    ...
    const resetPasswordMail = await this.repo.forgotPassword(email);
    const codeVerification = generateConfirmationCode();

    await this.mailRepo.sendMail(
      email,
      "Recuperação de Senha",
      `Utilize esse código para alterar sua senha: ${codeVerification}`
    );
