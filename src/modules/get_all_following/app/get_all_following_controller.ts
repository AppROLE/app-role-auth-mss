...
import { GetAllFollowingUseCase } from "./get_all_following_usecase";
...
import { GetAllFollowingViewModel } from "./get_all_following_viewmodel";
import { User } from "src/shared/domain/entities/user";

export class GetAllFollowingController {
    constructor (private usecase: GetAllFollowingUseCase) {}
    ...
    async handle(req: IRequest, requesterUser: Record<string, any>) {
        try {
            ...
            return new OK(
                users.map((user: User) => new GetAllFollowingViewModel(user.userName, user.userId, user.userProfilePhoto)).map(user => user.toJSON())
            ) 
        } catch (error: any) {
